...
with tab_symbol:
    ...
    period = st.selectbox(
        "الفترة",
        ["آخر 24 ساعة", "آخر 3 أيام", "آخر أسبوع"],
        key="symbol_period",
    )
    ...
    if st.button(
        ...
        if not symbol:
            ...
            try:
                with st.spinner("جارٍ جمع الأخبار وتحليلها..."):
    ...
    if result:
        st.write(f'**عدد الأخبار:** {result.get("count", 0)}')
        st.write(f'**متوسط النظام:** {result.get("system_average", 0)}')
        st.write(f'**متوسط AI:** {result.get("ai_average", 0)}')
